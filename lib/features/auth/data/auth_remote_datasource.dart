...
class AuthRemoteDatasource {
  ...
  Future<String?> resendVerificationCode(String email) async {
    try {
      final response = await _dio.post('/auth/resend-email-verification-code', data: {
        'email': email,
      });
  ...
  Future<String?> sendRecoveryEmail(String email) async {
    try {
      final response = await _dio.post('/auth/send-reset-password-email', data: {
        'email': email,
      });
      ...
      if (response.statusCode == 200) {
        ...
        return response.data['message'] ?? 'Error al enviar correo de recuperación.';
      }
    } on DioException catch (e) {
      final responseData = e.response?.data;
      if (responseData is Map<String, dynamic>) {
        return responseData['message'] ?? 'Error al enviar correo de recuperación.';
      } else {
  ...
  Future<String?> resetPassword(String email, String code, String newPassword) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return response.data['message'] ?? 'Error al restablecer la contraseña.';
      }
    } on DioException catch (e) {
      final responseData = e.response?.data;
      if (responseData is Map<String, dynamic>) {
