...
class ChatGroupsCreateForm extends HookWidget {
  ...
  void _createGroup(
    ...
    context.read<CreateChatGroupCubit>().createChatGroup(
      groupName,
      groupDescription,
      contacts.map((contact) => contact.userId).toList(),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => sl<CreateChatGroupCubit>(),
      child: BlocListener<CreateChatGroupCubit, CreateChatGroupState>(
        listener: (context, state) {
          if (state is CreateChatGroupSuccessState) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Grupo creado exitosamente")),
            );
            Navigator.of(context).pop();
          } else if (state is CreateChatGroupErrorState) {
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                ...
                child: BlocBuilder<CreateChatGroupCubit, CreateChatGroupState>(
                      ...
                      onPressed:
                          (selectedContacts.value.isNotEmpty &&
                                  ...
                                  createChatGroupState
                                      is CreateChatGroupLoadingState
                              ? () {
                                _createGroup(
                                  context,
                              ...
                              : null,
                      child:
                          createChatGroupState is CreateChatGroupLoadingState
                              ? const SizedBox(
                                width: 20,
                                height: 20,
